...
> = {
  Оплачен: { color: SUCCESS, fade: "var(--c-success-fade)" },
  "В долг": { color: WARN, fade: "var(--c-warn-fade)" },
  Возврат: { color: DANGER, fade: "var(--c-danger-fade)" },
};

const FILTERS: Array<"Все" | Sale["status"]> = [
  "Все",
  "Оплачен",
  "В долг",
  "Возврат",
...
function statusIcon(s: Sale["status"]) {
  if (s === "Оплачен") return <ArrowUpIcon size={18} />;
  if (s === "В долг") return <ClockIcon />;
  return <RefundIcon />;
}
...
export default function SalesPage() {
  ...
  const totals = useMemo(() => {
    const paid = sales
      .filter((s) => s.status === "Оплачен")
      .reduce((sum, s) => sum + s.amount, 0);
    const debt = sales
      .filter((s) => s.status === "В долг")
      .reduce((sum, s) => sum + s.amount, 0);
    const refund = sales
      .filter((s) => s.status === "Возврат")
      .reduce((sum, s) => sum + s.amount, 0);
    const net = paid - refund;
    return { paid, debt, refund, net };
  }, []);
  ...
  const visible = useMemo(() => {
    ...
    return [...sales]
      ...
      .filter((s) => {
        if (filter !== "Все" && s.status !== filter) return false;
        if (!q) return true;
        return (
    ...
    <div className="min-h-full" style={{ background: BG }}>
      ...
      <div className="px-5 pt-3">
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-2 mt-3.5">
            <Mini label="Оплачено" value={totals.paid} color={SUCCESS} />
            <Mini label="В долг" value={totals.debt} color={WARN} />
            <Mini label="Возвраты" value={totals.refund} color={DANGER} />
          </div>
        </div>
      </div>
      ...
      <div className="px-5 pb-6 flex flex-col gap-[18px]">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-baseline justify-between gap-2">
                        ...
                        <div
                          ...
                          style={{ color: isRefund ? DANGER : TEXT }}
                        >
                          {sign}
        ...
        {groups.length === 0 && (
          ...
          >
            <div className="text-[14px]">Продаж не найдено</div>
          </div>
        )}
      </div>
    </div>
