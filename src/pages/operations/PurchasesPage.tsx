...
> = {
  Принят: { color: SUCCESS, fade: "var(--c-success-fade)" },
  Ожидается: { color: WARN, fade: "var(--c-warn-fade)" },
  Отменён: { color: DANGER, fade: "var(--c-danger-fade)" },
};

const FILTERS: Array<"Все" | Purchase["status"]> = [
  "Все",
  "Принят",
  "Ожидается",
  "Отменён",
...
function statusIcon(s: Purchase["status"]) {
  if (s === "Принят") return <CheckIcon />;
  if (s === "Ожидается") return <ClockIcon />;
  return <XIcon />;
}
...
export default function PurchasesPage() {
  ...
  const totals = useMemo(() => {
    const accepted = purchases
      .filter((p) => p.status === "Принят")
      .reduce((sum, p) => sum + p.amount, 0);
    const pending = purchases
      .filter((p) => p.status === "Ожидается")
      .reduce((sum, p) => sum + p.amount, 0);
    const cancelled = purchases
      .filter((p) => p.status === "Отменён")
      .reduce((sum, p) => sum + p.amount, 0);
    return { accepted, pending, cancelled, total: accepted + pending };
  }, []);
  ...
  const visible = useMemo(() => {
    ...
    return [...purchases]
      ...
      .filter((p) => {
        if (filter !== "Все" && p.status !== filter) return false;
        if (!q) return true;
        return (
    ...
    <div className="min-h-full" style={{ background: BG }}>
      ...
      <div className="px-5 pt-3">
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-2 mt-3.5">
            <Mini label="Принято" value={totals.accepted} color={SUCCESS} />
            <Mini label="Ожидается" value={totals.pending} color={WARN} />
            <Mini label="Отменено" value={totals.cancelled} color={DANGER} />
          </div>
        </div>
      </div>
      ...
      <div className="px-5 pb-1 flex flex-col gap-[18px]">
          ...
          <div key={day}>
            ...
            >
              {ops.map((p, i) => {
                ...
                const isCancelled = p.status === "Отменён";
                return (
                  <div
                    key={p.id}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-[3px]">
                        <span
                          className="text-[12.5px] tabular-nums flex-1"
                          style={{ color: MUTED }}
                        >
                          {formatTime(p.date)}
                        </span>
        ...
        {groups.length === 0 && (
          ...
          >
            <div className="text-[14px]">Закупок не найдено</div>
          </div>
        )}
      </div>
    </div>
